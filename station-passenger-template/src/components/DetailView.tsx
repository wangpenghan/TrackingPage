        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-[#E5E7EB]">
          <div className="flex items-center gap-3">
            ...
            <span className={cn(
              ...
              statusClass === 'orange' ? 'bg-[#FFF8E6] text-[#FA8C16] border border-[#FFE7BA]' :
...
// ===== 工具函数 =====
function getTemplateStatus(template: TemplateData): 'red' | 'orange' | 'blue' | 'white' {
  if (!template.confirmed && !template.synced) return 'red'
  if (!template.confirmed && template.synced) return 'orange'
...
function getTrainTypeTag(type: string): 'primary' | 'warning' | 'info' {
  switch (type) {
    case '始发': return 'primary'
    case '终到': return 'warning'
    default: return 'info'
  }
}
